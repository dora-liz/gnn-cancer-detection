...
REQUIRED FILES (must be in the same directory as this script):
    - gnn_model.py                      : Model architecture definitions
    - train_data.pt                     : Base graph with drug/cell line structure
    - training_artifacts.pt             : Model hyperparameters and feature dimensions
    - feature_projector_trained_cpu.pt  : Trained feature projector weights
    - gnn_model_trained_cpu.pt          : Trained GNN encoder weights
    - link_predictor_trained_cpu.pt     : Trained link predictor (decoder) weights
...
INPUT FORMAT:
    ...
    Expected number of features: 19,215 (will be validated against trained model)
...
# Import model architectures (must be in same directory)
...
try:
    ...
    print(f"ERROR: Could not import gnn_model.py from {SCRIPT_DIR}")
    print("Make sure gnn_model.py is in the same directory as this script.")
    sys.exit(1)
...
REQUIRED_FILES = [
    "gnn_model.py",
    "train_data.pt",
    "training_artifacts.pt",
    "feature_projector_trained_cpu.pt",
    "gnn_model_trained_cpu.pt",
    "link_predictor_trained_cpu.pt",
]
...
def check_required_files(base_dir: Path) -> bool:
    ...
    if missing:
        ...
        for f in missing:
            print(f"  - {f}")
        print(f"\nMake sure all files are in: {base_dir}")
        return False
    return True
...
def main():
    ...
    base_graph = torch.load(model_dir / "train_data.pt", map_location="cpu", weights_only=False)
    artifacts = torch.load(model_dir / "training_artifacts.pt", map_location="cpu", weights_only=False)
    ...
    projector.load_state_dict(
        torch.load(model_dir / "feature_projector_trained_cpu.pt", map_location="cpu", weights_only=False)
    )
    model.load_state_dict(
        torch.load(model_dir / "gnn_model_trained_cpu.pt", map_location="cpu", weights_only=False)
    )
    decoder.load_state_dict(
        torch.load(model_dir / "link_predictor_trained_cpu.pt", map_location="cpu", weights_only=False)
    )
