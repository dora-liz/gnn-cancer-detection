...
Then send POST request:
    curl -X POST http://localhost:5000/predict \
         ...
         -d '{"gene_expression": [0.1, 0.2, ...]}'  # 19,215 values
"""
...
def load_models():
    ...
    base_graph = torch.load("train_data.pt", map_location="cpu", weights_only=False)
    artifacts = torch.load("training_artifacts.pt", map_location="cpu", weights_only=False)
    ...
    # Load weights
    projector.load_state_dict(torch.load("feature_projector_trained_cpu.pt", map_location="cpu", weights_only=False))
    model.load_state_dict(torch.load("gnn_model_trained_cpu.pt", map_location="cpu", weights_only=False))
    decoder.load_state_dict(torch.load("link_predictor_trained_cpu.pt", map_location="cpu", weights_only=False))
